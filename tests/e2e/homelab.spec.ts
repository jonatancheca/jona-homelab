...
test('cards and dialogs fit 320, 390 and 1280px, including long names', async ({ page, request }, testInfo) => {
  ...
  await page.setViewportSize({ width: 1280, height: 900 })
  await page.getByRole('button', { name: 'Add device', exact: true }).click()
  const desktopDialog = page.getByRole('dialog', { name: 'Add device' })
  const desktopFields = desktopDialog.locator('.device-fields')
  expect(await desktopFields.evaluate(element => getComputedStyle(element).gridTemplateColumns.split(' ').length)).toBe(2)
  const desktopInputs = desktopDialog.locator('label.field')
  expect((await desktopInputs.nth(0).boundingBox())?.y).toBe((await desktopInputs.nth(1).boundingBox())?.y)
  expect((await desktopInputs.nth(2).boundingBox())?.y).toBe((await desktopInputs.nth(3).boundingBox())?.y)
  await desktopDialog.getByRole('button', { name: 'Cancel' }).click()
...
test('registers Wake-on-LAN-only devices without remote configuration', async ({ page, request }) => {
  ...
  await page.goto('/')
  const card = page.getByRole('article', { name, exact: true })
  await expect(card.getByText('Not checked', { exact: true })).toHaveCount(0)
  await expect(card.getByText('Wake-on-LAN only', { exact: true })).toHaveCount(0)
  await expect(card.getByText('Wake-on-LAN only. Add an address to check status.', { exact: true })).toHaveCount(0)
