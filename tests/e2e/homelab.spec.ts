...
test('registers Wake-on-LAN-only devices without remote configuration', async ({ page, request }) => {
  const headers = { 'content-type': 'application/json' }
  const name = 'Wake-only PC'
  const created = await request.post('/api/devices', { headers, data: { name, mac: 'AA:BB:CC:DD:EE:08', remoteMethod: 'none' } })
  expect(created.status()).toBe(201)
  const device = await created.json()
  expect(device.remoteMethod).toBe('none')
  expect(device.address).toBeNull()
  expect(device.sshUser).toBeNull()
  await page.route('**/api/devices/status', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([{ deviceId: device.id, networkReachable: false, remoteReady: false, remoteMethod: 'none', checkedAt: new Date().toISOString() }]) }))
  await page.goto('/')
  const card = page.getByRole('article', { name, exact: true })
  await expect(card.getByText('Not checked', { exact: true })).toBeVisible()
  await expect(card.getByText('Wake-on-LAN only', { exact: true })).toBeVisible()
  await expect(card.getByText('Wake-on-LAN only. Add an address to check status.', { exact: true })).toBeVisible()
  await expect(card.getByRole('button', { name: 'Shut down', exact: true })).toHaveCount(0)
  await card.getByRole('button', { name: `Edit ${name}` }).click()
  const form = page.getByRole('dialog', { name: 'Edit device' })
  await expect(form.getByRole('radio', { name: /Wake-on-LAN only/ })).toBeChecked()
  expect(await form.getByRole('textbox', { name: 'Private IPv4 or machine name (optional)' }).getAttribute('required')).toBeNull()
  await expect(form.getByRole('textbox', { name: 'SSH user' })).toHaveCount(0)
  await expect(form.getByRole('textbox', { name: 'Companion pairing code' })).toHaveCount(0)
  await form.getByRole('button', { name: 'Cancel' }).click()
  await request.delete(`/api/devices/${device.id}`, { headers, data: {} })
})
